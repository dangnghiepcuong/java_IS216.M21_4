...
--------------------------------------------------------
--  DDL for Procedure REG_UPDATE_TIME
--------------------------------------------------------


  CREATE OR REPLACE EDITIONABLE PROCEDURE "REG_UPDATE_TIME" 
(PAR_PERSONALID PERSON.ID%TYPE, PAR_SCHEDID SCHEDULE.ID%TYPE , PAR_TIME REGISTER.TIME%TYPE
) 
AS
    var_OldTime REGISTER.Time%type;
    set_NO REGISTER.NO%type;
BEGIN
    --select out the time, which has been registered
    select Time into var_OldTime
    from REGISTER REG
    where REG.PersonalID = par_PersonalID
    and REG.SchedID = par_SchedID;
    
    --if the new time = old time then return;
    if (var_OldTime = par_Time)
    then
        return;
    end if;
    
    --set NO for new time registion in schedule
    set_NO := REG_SIGNED_NO(par_PersonalID, par_SchedID, par_Time);
    
    --If available then update time registion
    if (set_NO > 0)
    then
        --inc a registion in new time sched, 
        --dec a registion in old time sched
        SCHED_INC_REG(par_SchedID, par_Time);
        SCHED_DEC_REG(par_SchedID, par_Time);
